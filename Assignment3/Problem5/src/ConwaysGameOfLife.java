...
public class ConwaysGameOfLife
{
    ...
    {
        ImmutablePair<Integer, BiFunction<Boolean, Integer, Boolean>> iterationCountAndRulePair = determineArguments(args);
        ...
        else	
        {
            ...
            BiFunction<Boolean, Integer, Boolean> livenessRule = iterationCountAndRulePair.getRight();
            System.out.println(livenessRule);
    ...
    int [][]runScenario(int countSteps, BiFunction<Boolean, Integer, Boolean> ruleComputeLiveness)
    {
    ...
    //		returns a pointer to the final state of the model, encoded in the same row-major order as was bufferEven.  This final state was produced by 
    //		running Conway's Game of Life for countSteps, starting with the initial state of the model
    int [][]updateSpace(int iTime, BiFunction<Boolean, Integer, Boolean> ruleComputeLiveness)
    {
    ...
    //			
    void updateCell(int [][]currentCells, int [][]nextCells, int row, int col, BiFunction<Boolean, Integer, Boolean> ruleComputeLiveness)
    {
    ...
    ImmutablePair<Integer, BiFunction<Boolean, Integer, Boolean>>  determineArguments(String args[])
    {
        int countArgumentsNotIncludingFilename = args.length;

        //Define our functions to be returned
        ...
        BiFunction<Boolean, Integer, Boolean> alive2to4Func = (isCurrentCellAlive, countSurroundingAlive) -> {
            if (isCurrentCellAlive)
        ...
        BiFunction<Boolean, Integer, Boolean> alive1to3Func = (isCurrentCellAlive, countSurroundingAlive) -> {
            if (isCurrentCellAlive)
        ...
        if (countArgumentsNotIncludingFilename == 0)
        {
            ...
            return(new ImmutablePair<>(DEFAULT_ITERATION_COUNT, classicFunc));
        }
        else 
        {
            try
            {
                ...
                if (countArgumentsNotIncludingFilename == 1)
                    ...
                    if (args[1] == "Classic")
                        return(new ImmutablePair<>(countIterations, classicFunc));
                    if (args[1] == "Liveness2To4")
                        return(new ImmutablePair<>(countIterations, alive2to4Func));
                    if (args[1] == "Liveness1To3")
                        return(new ImmutablePair<>(countIterations, alive1to3Func));
                    else
                        return(null);
                    }
                else
