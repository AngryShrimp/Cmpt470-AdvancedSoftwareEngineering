...
int main(int argc, char *argv[])
{
  ...
  else	
  {
	  ...
	  int *finalState = runScenario(countIterations, 0);	
...
//		ruleSet is an integer used to determine which ruleset to follow
// POSTCONDITIONS:
...
int *runScenario(int countSteps, int ruleSet)
{
	...
	for (int iTime  = 0; iTime < countSteps; iTime ++)
		{
		arrayLatestState = updateSpace(iTime, ruleSet);
...
int *updateSpace(int iTime, int ruleSet)
{
	...
	for (int row = 0; row < ROW_COUNT; row++)
		for (int col = 0; col < COL_COUNT; col++)
		  {
		    // update nextCells
			updateCell(currentCells, nextCells, row, col, ruleSet);	// another (less efficient, but still relatively clean) alternative would be to test here if it's an even or odd step, and to call "update cell" with different swapps first two arguments based on the outcome
		  }
...
void updateCell(int *currentCells, int *nextCells, int row, int col, int ruleSet)
{
	...
	nextCells[iCurrentCell] = computeLiveness(isCurrentCellAlive, countSurroundingAlive, ruleSet);
}
...
//		ruleSet is an integer used to determine which ruleset to follow
//			0: 			Default Conway's game of life
//			1: 			Modified Game of life that changes the cells to live if there are between 2 and 4 nearby cells and to create a new cell if there are 2 cells nearby
//			default:	Defaults to the original game of life if there is a number that isnt recognized
// POSTCONDITIONS:
//		returns 1 if the cell is live in the next iteration, 0 otherwise 
int computeLiveness(bool isCurrentCellAlive, int countSurroundingAlive, int ruleSet)
{
	switch(ruleSet)
	{
		case(0):
		...
		case(1):
			if (isCurrentCellAlive)
				...
				return (countSurroundingAlive == 2);
		//Deafult if the number isnt recognized
		default:
			if (isCurrentCellAlive)
				return (countSurroundingAlive >= 2 && countSurroundingAlive <= 3);
			else
...
//		Guarantees that textualEncodingForCellValue(cellValueForTextualEncoding(cEncoded)) == cEncoded
int cellValueForTextualEncoding(char cEncoded)
{
  return cEncoded == '*' ? 1 : 0;
}	
